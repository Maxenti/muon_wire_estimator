...
def plot_quantity(
    ...
    *,
    ...
    fig, axes = plt.subplots(len(wire_names), 1, figsize=(10, 4 * len(wire_names)), squeeze=False)
    axes = axes[:, 0]
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Plot visibility scan results from summary CSV."
    )
    parser.add_argument("--summary-csv", type=Path, required=True, help="Summary CSV from scan_visibility.py")
    parser.add_argument("--output-dir", type=Path, required=True, help="Directory for plot PNGs")
    ...
    plot_quantity(
        ...
        title_prefix="Deterministic peak voltage",
        use_log_y=False,
    )
    plot_quantity(
        rows,
    ...
    plot_quantity(
        ...
        title_prefix="Deterministic collected primary electrons",
        use_log_y=False,
    )
    plot_quantity(
        rows,
        quantity_key="det_drift_time_ns",
        ylabel="Drift time [ns]",
        output_path=output_dir / "det_drift_time_ns_linear.png",
        title_prefix="Deterministic drift time",
        use_log_y=False,
    )
    ...
    plot_quantity(
        ...
        ylabel="Stochastic mean peak voltage [mV]",
    ...
    plot_quantity(
        ...
        ylabel="Stochastic median peak voltage [mV]",
    ...
    plot_quantity(
        ...
        ylabel="Stochastic max peak voltage [mV]",
